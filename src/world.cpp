...
void world::rodBoundaryCondition() {
    ...
    rod->setVertexBoundaryCondition(rod->getVertex(start_end), start_end);
    rod->setVertexBoundaryCondition(rod->getVertex(start_end+1), start_end+1);
    rod->setThetaBoundaryCondition(0.0, start_end);
    rod->setVertexBoundaryCondition(rod->getVertex(last_end), last_end);
    rod->setVertexBoundaryCondition(rod->getVertex(last_end+1), last_end+1);
    rod->setThetaBoundaryCondition(0.0, last_end);

    start_end = 135;
    last_end = 160;
    int  mid_point = 149;
    rod->setVertexBoundaryCondition(rod->getVertex(mid_point), mid_point);
//    rod->setVertexBoundaryCondition(rod->getVertex(start_end), start_end);
//    rod->setVertexBoundaryCondition(rod->getVertex(start_end+1), start_end+1);
//    rod->setThetaBoundaryCondition(0.0, start_end);
//    rod->setVertexBoundaryCondition(rod->getVertex(last_end), last_end);
...
void world::updateBoundary() {
    ...
    if (currentTime < wait_time) {
        rod->setVertexBoundaryCondition(rod->getVertex(mid_point) + u_down * deltaTime, mid_point);
    }

    if (currentTime > wait_time && currentTime <= wait_time + pull_time) {   // Pulling
        rod->setVertexBoundaryCondition(rod->getVertex(start_end) - u * deltaTime, start_end);
        rod->setVertexBoundaryCondition(rod->getVertex(start_end+1) - u * deltaTime, start_end+1);
        rod->setVertexBoundaryCondition(rod->getVertex(last_end) + u * deltaTime, last_end);
        rod->setVertexBoundaryCondition(rod->getVertex(last_end+1) + u * deltaTime, last_end+1);
//
//        rod->setVertexBoundaryCondition(rod->getVertex(start_end2) - u * deltaTime, start_end2);
//        rod->setVertexBoundaryCondition(rod->getVertex(start_end2+1) - u * deltaTime, start_end2+1);
//        rod->setVertexBoundaryCondition(rod->getVertex(last_end2) + u * deltaTime, last_end2);
//        rod->setVertexBoundaryCondition(rod->getVertex(last_end2+1) + u * deltaTime, last_end2+1);
...
void world::calculateForce() {
    ...
    temp[0] = stepper->force[start_end*4] + stepper->force[start_end*4+4];
    temp[1] = stepper->force[start_end*4+1] + stepper->force[start_end*4+5];
    temp[2] = stepper->force[start_end*4+2] + stepper->force[start_end*4+6];

    temp1[0] = stepper->force[last_end*4] + stepper->force[last_end*4+4];
    temp1[1] = stepper->force[last_end*4+1] + stepper->force[last_end*4+5];
    temp1[2] = stepper->force[last_end*4+2] + stepper->force[last_end*4+6];
}
